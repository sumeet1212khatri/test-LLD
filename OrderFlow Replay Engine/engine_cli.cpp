...
// Simple key-value JSON extractor (no deps)
...
static void init_feed() {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        cfg.ticks  = 3000; cfg.volatility = 0.0008;
        cfg.spread_bps = 3.0; cfg.seed = 42 + i;
...
static std::string handle(const std::string& cmd, const std::string& payload) {
    ...
    if (cmd == "reset") {
        g_trade_log.clear(); g_order_log.clear();
        init_sim(); init_feed();
        return "{\"ok\":true}";
    }
    ...
    if (cmd == "positions") {
        ...
        for (auto& [sym, p] : pos) {
            if (!first) r += ",";
            r += position_to_json(p); first = false;
        }
        return r + "]";
    }
    ...
    if (cmd == "trades") {
        std::string r = "[";
        bool first = true;
        for (auto& t : g_trade_log) { if (!first) r += ","; r += t; first = false; }
        return r + "]";
    }
    ...
    if (cmd == "orders") {
        std::string r = "[";
        bool first = true;
        for (auto& o : g_order_log) { if (!first) r += ","; r += o; first = false; }
        return r + "]";
    }
    ...
    if (cmd == "order") {
        ...
        try {
            double px = std::stod(jget(payload, "price"));
            order.price = to_price(px);
    ...
    if (cmd == "feed_step") {
        int n = 10;
        try { n = std::stoi(jget(payload, "n")); } catch(...) {}
        n = std::min(n, 200);
    ...
    if (cmd == "backtest") {
        ...
        MarketDataParser::Config cfg;
        cfg.symbol = symbol; cfg.start_price = start_price;
        cfg.ticks = ticks; cfg.volatility = 0.001; cfg.seed = 777;
        auto bt_ticks = MarketDataParser::generate_synthetic(cfg);

        RiskLimits rl;
        rl.max_order_qty = 10000; rl.max_position = 500000;
        rl.max_notional_usd = 1e9; rl.max_orders_per_sec = 10000;

        Backtester bt(rl);
        bt.add_symbol(symbol);

        std::unique_ptr<IStrategy> strat;
        if (strategy_name == "Momentum")       strat = std::make_unique<MomentumStrategy>();
        else if (strategy_name == "MeanReversion") strat = std::make_unique<MeanReversionStrategy>();
        else                                    strat = std::make_unique<MarketMakingStrategy>();
